...
struct sdrstate {
  ...
  double samprate;  // True sample rate of single A/D converter
  double reference; // reference oscillator frequency
...
static void rx888_set_samprate(struct sdrstate *sdr,double samprate,double reference);
...
int main(int argc,char *argv[]){
  ...
  // Attenuation, default 0
  {
    ...
    double samprate = config_getdouble(Dictionary,Name,"samprate",32000000.0);
    double const minsamprate = 1000000.0; // 1 MHz?
    if(samprate < minsamprate){
      fprintf(stdout,"Invalid sample rate %'lf, forcing %'lf\n",samprate,minsamprate);
      samprate = minsamprate;
    }
    // Reference oscillator frequency, default 27000000 (27 MHz)
    double const reference = config_getdouble(Dictionary,Name,"reference",27000000.0);
    rx888_set_samprate(sdr,samprate,reference);
  }
  
  fprintf(stdout,"Samprate %'lf, Reference %'lf, Gain %.1f dB, Attenuation %.1f dB, Dithering %d, Randomizer %d, USB Queue depth %d, USB Request size %d * pktsize %d = %'d bytes\n",
	  sdr->samprate,sdr->reference,sdr->rf_gain,sdr->rf_atten,sdr->dither,sdr->randomizer,sdr->queuedepth,sdr->reqsize,sdr->pktsize,sdr->reqsize * sdr->pktsize);
...
static void rx888_set_samprate(struct sdrstate *sdr,double samprate,double reference){
  ...
  if (r_samprate < 1e6) {
    fprintf(stdout,"ERROR - requested sample rate is too low: %'lg\n",samprate);
    return;
  }
  ...
  double const actual_samprate = reference * actual_ratio / output_ms / pow(2.0,rdiv);
  fprintf(stdout,"Samprate %'lf, Reference %'lf, A=%d B=%d C=%d output_ms=%d rdice=%d Actual Samprate %'lf\n",
	  sdr->samprate,sdr->reference,a,b,c,output_ms,rdiv,actual_samprate);
